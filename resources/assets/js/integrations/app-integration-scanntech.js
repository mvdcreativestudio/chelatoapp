document.addEventListener('DOMContentLoaded', () => {
      ...
      if (this.checked) {
        Swal.fire({
          ...
          preConfirm: () => {
            ...
            if (!systemId || !branchId || !company) {
              Swal.showValidationMessage('Todos los campos son obligatorios');
              return null;
            }
            return { systemId, branchId, company };
          },
          ...
          if (result.isConfirmed) {
            const { systemId, branchId, company } = result.value;
            card.classList.add('active-integration');
            activateScanntech(storeId, systemId, company, branchId, this); // Enviar datos al servidor
          } else {
            this.checked = false;
          }
        });
  ...
  function activateScanntech(storeId, systemId, company, branchId, switchEl) {
    $.ajax({
