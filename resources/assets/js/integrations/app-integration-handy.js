document.addEventListener('DOMContentLoaded', () => {
  // Manejo de los switches para Handy
  const handySwitches = document.querySelectorAll('[id^="handySwitch-"]');
  handySwitches.forEach(switchEl => {
    switchEl.addEventListener('change', function () {
      const storeId = this.dataset.storeId;
      const card = document.getElementById(`handy-card-${storeId}`);
      if (!card) {
        console.error(`No se encontró la tarjeta Handy con ID: handy-card-${storeId}`);
        return;
      }
      ...
      if (this.checked) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            activateHandy(storeId, systemId, branchId, this); // Enviar datos al servidor
          } else {
            this.checked = false;
          }
        });
        ...
        deactivateHandy(storeId, this);
      }
    });
  });

  // Delegación de eventos para agregar nuevas filas en el modal Handy
  document.querySelectorAll('.modal[data-provider="handy"]').forEach((modal, modalIndex) => {
    modal.addEventListener('click', function (event) {
      if (event.target.classList.contains('add-terminal-row') || event.target.closest('.add-terminal-row')) {
        ...
        const terminalsTable = modal.querySelector(
          `.terminals-table[data-provider="handy"][data-store-id="${storeId}"]`
        );

        if (!terminalsTable) {
          console.error(`No se encontró la tabla de terminales para el proveedor: handy, tienda: ${storeId}`);
          return;
        }
  ...
  // Botón de guardar cambios para Handy
  document.querySelectorAll('.save-terminals[data-provider="handy"]').forEach(button => {
    button.addEventListener('click', function () {
      ...
      terminalsTable.querySelectorAll('tr').forEach((row, index) => {
        ...
        if (name && identifier) {
          terminals.push({
            ...
            pos_provider_id: 3 // Identificador del proveedor (Handy)
          });
        } else {
      ...
      if (terminals.length === 0) {
        ...
        bootstrap.Modal.getInstance(document.querySelector(`#handyModal-${storeId}`)).hide();
      ...
      const modalContent = document.querySelector(`#handyModal-${storeId} .modal-content`);
  ...
  // Función para activar Handy
  function activateHandy(storeId, systemId, branchId, switchEl) {
    $.ajax({
      url: `${window.baseUrl}admin/integrations/${storeId}/handy`,
      type: 'POST',
      data: {
        accepts_handy: 1,
        system_id: systemId,
      ...
      success: function (response) {
        if (response.success) {
          ...
          console.error('Error al activar Handy:', response.message);
          handleError(response.message, switchEl, false);
        }
      },
      error: function (xhr) {
        console.error('Error en el servidor al activar Handy:', xhr.responseJSON?.message);
        handleError(xhr.responseJSON?.message || 'Error al activar Handy', switchEl, false);
      }
    });
  }

  // Función para desactivar Handy
  function deactivateHandy(storeId, switchEl) {
    $.ajax({
      url: `${window.baseUrl}admin/integrations/${storeId}/handy`,
      type: 'POST',
      data: {
      ...
      success: function (response) {
        if (response.success) {
          ...
          console.error('Error al desactivar Handy:', response.message);
          handleError(response.message, switchEl, false);
        }
      },
      error: function (xhr) {
        console.error('Error en el servidor al desactivar Handy:', xhr.responseJSON?.message);
        handleError(xhr.responseJSON?.message || 'Error al desactivar Handy', switchEl, true);
      }
    });
  }
