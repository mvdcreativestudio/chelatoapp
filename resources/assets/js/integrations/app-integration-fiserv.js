document.addEventListener('DOMContentLoaded', () => {
  // Manejo de los switches para Fiserv
  const fiservSwitches = document.querySelectorAll('[id^="fiservSwitch-"]');
  fiservSwitches.forEach(switchEl => {
    switchEl.addEventListener('change', function () {
      const storeId = this.dataset.storeId;
      const card = document.getElementById(`fiserv-card-${storeId}`);
      if (!card) {
        console.error(`No se encontró la tarjeta Fiserv con ID: fiserv-card-${storeId}`);
        return;
      }

      if (this.checked) {
        Swal.fire({
          title: 'Activar Fiserv',
          html: `
          ...
          if (result.isConfirmed) {
            ...
            activateFiserv(storeId, systemId, branchId, this); // Enviar datos al servidor
          } else {
            this.checked = false;
          }
        });
        ...
        deactivateFiserv(storeId, this);
      }
    });
  });

  // Delegación de eventos para agregar nuevas filas en el modal Fiserv
  document.querySelectorAll('.modal[data-provider="fiserv"]').forEach((modal, modalIndex) => {
    modal.addEventListener('click', function (event) {
      if (event.target.classList.contains('add-terminal-row') || event.target.closest('.add-terminal-row')) {
        ...
        const terminalsTable = modal.querySelector(
          `.terminals-table[data-provider="fiserv"][data-store-id="${storeId}"]`
        );

        if (!terminalsTable) {
          console.error(`No se encontró la tabla de terminales para el proveedor: fiserv, tienda: ${storeId}`);
          return;
        }
  ...
  // Botón de guardar cambios para Fiserv
  document.querySelectorAll('.save-terminals[data-provider="fiserv"]').forEach(button => {
    button.addEventListener('click', function () {
      ...
      terminalsTable.querySelectorAll('tr').forEach((row, index) => {
        ...
        if (name && identifier) {
          terminals.push({
            ...
            pos_provider_id: 2 // Identificador del proveedor (Fiserv)
          });
        } else {
      ...
      if (terminals.length === 0) {
        ...
        bootstrap.Modal.getInstance(document.querySelector(`#fiservModal-${storeId}`)).hide();
      ...
      const modalContent = document.querySelector(`#fiservModal-${storeId} .modal-content`);
  ...
  // Función para activar Fiserv
  function activateFiserv(storeId, systemId, branchId, switchEl) {
    $.ajax({
      url: `${window.baseUrl}admin/integrations/${storeId}/fiserv`,
      type: 'POST',
      data: {
        accepts_fiserv: 1,
        system_id: systemId,
      ...
      success: function (response) {
        if (response.success) {
          ...
          console.error('Error al activar Fiserv:', response.message);
          handleError(response.message, switchEl, false);
        }
      },
      error: function (xhr) {
        console.error('Error en el servidor al activar Fiserv:', xhr.responseJSON?.message);
        handleError(xhr.responseJSON?.message || 'Error al activar Fiserv', switchEl, false);
      }
    });
  }

  // Función para desactivar Fiserv
  function deactivateFiserv(storeId, switchEl) {
    $.ajax({
      url: `${window.baseUrl}admin/integrations/${storeId}/fiserv`,
      type: 'POST',
      data: {
      ...
      success: function (response) {
        if (response.success) {
          ...
          console.error('Error al desactivar Fiserv:', response.message);
          handleError(response.message, switchEl, false);
        }
      },
      error: function (xhr) {
        console.error('Error en el servidor al desactivar Fiserv:', xhr.responseJSON?.message);
        handleError(xhr.responseJSON?.message || 'Error al desactivar Fiserv', switchEl, true);
      }
    });
  }
