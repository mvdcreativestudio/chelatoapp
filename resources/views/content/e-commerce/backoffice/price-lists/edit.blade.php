        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                response.products.forEach(function(product) {
                    var priceValue = product.price ? product.price : '';
                    productsList.append(`
                        <tr>
                            <td>${product.name}</td>
                            <td>
                                <div class="input-group">
                                    <span class="input-group-text">${product.currency === 'Dólar' ? 'USD' : 'UYU'}</span>
                                    <input type="number" 
                                        name="prices[${product.id}]" 
                                        class="form-control" 
                                        value="${priceValue ? parseFloat(priceValue).toFixed(2) : ''}" 
